...
一套部署允许创建多个家庭空间。家庭名称在部署内唯一，登录时使用家庭名称和共享口令定位空间；业务查询仍只从已验证 session 取得家庭 ID，客户端不能直接指定家庭 ID。创建家庭不再需要服务器初始化令牌，共享口令只要求非空，不要求长度或字符组合，仍使用 Argon2id 保存摘要。

所有 IP 地址都可以成为应用入口，不维护固定 IP 白名单。JSON 写请求仍限制为 32 KiB，并按当前请求地址执行动态同源校验；这允许局域网 IP、SSH 隧道和后续正式域名访问，同时拒绝第三方网站跨站提交。登录、创建和重置继续通过 Redis 做失败频率控制。
